...
public class LauncherSingleCollection {


    public static Thread[] initThreads(int start, int finish, int countThread){
        ...
        for (int i = 0; i < countThread; i++, start += unit, last--) {
            if (last > 0) {
                thread = new Thread(new PrimeExecutor(start, start + unit + 1));
                thread.start();
            } else {
                thread = new Thread(new PrimeExecutor(start, start + unit));
                thread.start();
            }
            arr[i] = thread;
        }
    ...
    public static void printPrimes() {
        for (Integer integer : PrimeExecutor.getPrimeNumbers()) {
            System.out.print(integer + " ");
        }
    }

    private static class PrimeExecutor implements Runnable {
        private int start;
        private int finish;
        private static List<Integer> primeNumbers  = Collections.synchronizedList(new ArrayList<>());

        public PrimeExecutor(int start, int finish){
            this.start = start;
            this.finish = finish;
        }
